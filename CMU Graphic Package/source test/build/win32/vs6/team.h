...
class Team
	...
	public:
		...
		void hasCaught(Team & otherTeam, Frisbee & disc, int caught, double & rotation, int pulled, int x1, int x2, int y1, int y2)   {
			for(int loop = 0; loop<numplayers; ++loop)  {		
				if( !caught && status!=3 && ((lastcaught != loop+1) || status !=1) && (!pulled || otherTeam.getInit()) )
					if(disc.hasCaught(myplayers[loop], status-1)) { 
						//if(myplayers[loop].hasCaught() )  {
							mycaught = (loop+1);
							lastcaught = (loop+1);
							selected = (loop);
							//change offense
		...
		void move(int number, int xamount, int yamount, int fieldw, int fieldh, int yard)  {
		...
		double findRotation()  {
			...
			myplayers[mycaught-1].giveCords(x1,y1,junk,junk);
			myplayers[selected].giveCords(x2,y2,junk,junk);
