...
class Player
	...
	public:
		...
		void giveCords(int & x, int & y, int & csize, int & rsize)  {
			x = xpos;
			y = ypos;
		...
		void drawPlayer(Window & window, int x, int y, int caught)   {
			int walk=0;
			if(animate < 10 && caught!=1)
				walk = 0;
			else if(animate >= 20 && caught!=1)
				animate = 0;
			else
				walk = 1;
			...
			if( Field::realx(xpos,ypos)+columnsize*size > x && (Field::realx(xpos,ypos) < x+window.getWidth()) ) {
				if( (ypos+rowsize*size > y) && (ypos < y+window.getHeight()) )  {
					if(direction)  {
						if(walk == 1)   
							window.drawImage(standright, Field::realx(xpos,ypos)-x, ypos-y, Transform(size, size));
						else
							window.drawImage(runright, Field::realx(xpos,ypos)-x, ypos-y, Transform(size, size));
					}
					else
						if(walk == 1)   
							window.drawImage(standleft, Field::realx(xpos,ypos)-x, ypos-y, Transform(size, size));
						else
							window.drawImage(runleft, Field::realx(xpos,ypos)-x, ypos-y, Transform(size, size));
				}
			}
		}
